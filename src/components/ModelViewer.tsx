...
interface ModelViewerProps {
  ...
  uploadPrompt?: string;
  // AGGIUNTA: callback per restituire dimensioni
  onDimensions?: (dims: { x: number; y: number; z: number }) => void;
}

// Usa window.matchMedia invece di navigator.userAgent
const isMobile = () => {
...
function Model({ file, fileType, onAnalysis, onDimensions }: ModelViewerProps) {
  ...
  useEffect(() => {
    ...
    // Loader in base al tipo: STLLoader o OBJLoader
    const loader = fileType === 'stl' ? new STLLoader() : new OBJLoader();

    // Annulla eventuale lettura in corso
  ...
  // Se geometry è un BufferGeometry (STL) lo mostri come <mesh>
  if (geometry instanceof THREE.BufferGeometry) {
    ...
    // Altrimenti (OBJ) lo mostriamo come <primitive>
    return (
      <primitive
        ref={modelRef}
        object={geometry}
